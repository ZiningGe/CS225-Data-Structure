...
class KDTree
{
  ...
  private:
     ...
     */
    Point<Dim> QuickSelect(vector<Point<Dim>>& vector, int start, int end, int rank, int dimension);

    int Partition(vector<Point<Dim>>& vector, int start, int end, int dimension);

    KDTreeNode* construct(vector<Point<Dim>>& vector, int start, int end, int dimension);

    void clear(KDTreeNode * subroot);

    void copy(KDTreeNode *& subroot, KDTreeNode * other);

    KDTreeNode* find(const Point<Dim>& query, int dimension, KDTreeNode* curRoot) const;
};
