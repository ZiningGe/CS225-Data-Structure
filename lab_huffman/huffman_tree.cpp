                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty()){
        ...
        TreeNode* single = singleQueue.front();
        TreeNode* merge = mergeQueue.front();
        if(single->freq.getFrequency() < merge->freq.getFrequency()){
            smallest = single;
            singleQueue.pop();
        }
        else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(!(mergeQueue.size() == 1 && singleQueue.empty())){
        ...
        TreeNode* temp = new TreeNode(left->freq.getFrequency()+right->freq.getFrequency());
        temp->left = left;
        temp->right = right;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    if(bfile.hasBits()){
        if(!bfile.getNextBit()){
            TreeNode* temp = new TreeNode(Frequency(0));
